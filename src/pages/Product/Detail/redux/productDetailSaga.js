...
function* workGetProductRequest({ payload }) {
  try {
    ...
    const response = yield call(getProductApiRequest, productId);
...
function* workGetCommentsRequest({ payload }) {
  try {
    ...
    yield delay(700);
    const { productId } = payload;
    const response = yield call(getProductCommentsApiRequest, productId);
...
function* workCreateCommentRequest({ payload }) {
  try {
    // TODO: Delete delay
    yield delay(700);
    const { commentDetail, productId } = payload;
    const response = yield call(createCommentApiRequest, {
      commentDetail, productId,
    });
